...
//#define SFQRedColor         [UIColor grayColor]
...
-(instancetype)initWithTitle:(NSString *)title message:(NSString *)message cancelBtnTitle:(NSString *)cancelTitle otherBtnTitle:(NSString *)otherBtnTitle clickIndexBlock:(LXAlertClickIndexBlock)block{
    if(self=[super init]){
        ...
        UIButton *cancelBtn = [[UIButton alloc] initWithFrame:CGRectMake(_alertView.bounds.size.width - 40, 8, 20, 20)];
        [cancelBtn setBackgroundImage:[UIImage imageNamed:@"calcelBtn"] forState:UIControlStateNormal];
        [cancelBtn setBackgroundImage:[UIImage imageNamed:@"calcelBtn"] forState:UIControlStateHighlighted];
        [cancelBtn addTarget:self action:@selector(cencelBtnDidClick) forControlEvents:UIControlEventTouchUpInside];
        [_alertView addSubview:cancelBtn];
        self.smallCancelBtn = cancelBtn;
      //  ************************************************************************************
